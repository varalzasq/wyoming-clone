...
function verifyPassword(plain: string, stored: string): boolean {
  ...
  const attempt = pbkdf2Sync(plain, salt, 210000, 64, "sha512").toString("hex");
  try {
...
export const mockAdminLogin = async (req: Request, res: Response): Promise<void> => {
...
export const mockAdminVerify = async (req: Request, res: Response): Promise<void> => {
  const secret = process.env.ADMIN_JWT_SECRET;
  if (!secret) {
